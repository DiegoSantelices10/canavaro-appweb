...
export const updatePrices = async (req, res) => {
    ...
    try {
        await Producto.updateMany({ 'imagen.url': '', 'imagen.public_id': '' }, { $unset: { imagen: 1 } });
        console.log('Campos de imagen vacíos eliminados exitosamente');
    } catch (error) {
        console.error('Error al eliminar campos de imagen vacíos:', error);
    }
